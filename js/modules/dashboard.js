...
export async function renderDashboard(container, appInstance) {
    ...
    try {
        ...
        window._dashboardFiltrarStatus = (status) => filtrarPorStatus(status, container, appInstance);
        window._dashboardVerDetalhes = (id) => verDetalhes(id, appInstance);
        window._dashboardVerAbordagem = (id, tipo) => verAbordagem(id, tipo, appInstance);
...
function configurarPullToRefresh(container, appInstance) {
    ...
    dashboardContent.addEventListener("touchstart", handleTouchStart, { passive: true });
...
function mostrarIndicadorPullRefresh(container, progress, loading = false) {
    ...
    const texto = percentual >= 100 ? "🔄 Solte para atualizar" : `⬇️ Puxe para atualizar (${percentual}%)`;
    const cor = percentual >= 100 ? "var(--verde-bandeira)" : "var(--cinza-medio)";

    indicator.innerHTML = `<span style="color:${cor}">${texto}</span>`;
    indicator.style.transform = `translateY(${-100 + progress * 100}%)`;
    indicator.style.background = progress >= 1 ? "var(--verde-muito-claro)" : "var(--branco)";
}
...
async function carregarOcorrencias() {
    ...
    try {
        ...
        // Buscar anexos e nome do criador para cada ocorrência
            ...
            (ocorrencias || []).map(async (occ) => {
                try {
                    ...
                    return {
                        ...occ,
                        anexos: anexos || [],
                        criador: { nome_completo: nomeCriador }
                    };
                } catch (e) {
                    return { 
                        ...occ, 
                        anexos: [],
                        criador: { nome_completo: "Desconhecido" }
                    };
                }
            })
        );

        estado.ocorrenciasData = ocorrenciasComDados || [];
...
function renderizarDashboard(container, appInstance) {
    ...
    const temNovidades = briefings?.ocorrencias?.length > 0 || briefings?.avisos?.length > 0;

    const totalAbordagens = (abordagens.veiculos || []).length + (abordagens.pessoas || []).length;
...
function renderOcorrenciasLista(ocorrencias, appInstance) {
    ...
    return ocorrencias.map((occ) => {
        ...
        if (occ.anexos && occ.anexos.length > 0) {
            ...
            const imagem = occ.anexos.find(a => a.tipo_arquivo === 'image' || a.tipo === 'image');
            if (imagem) {
                temImagem = true;
        ...
        const previewObs = occ.observacoes 
            ? occ.observacoes.substring(0, 60) + (occ.observacoes.length > 60 ? '...' : '')
            : '';
...
function renderAbordagensLista(abordagens, appInstance) {
    ...
    return todasAbordagens.map((item) => {
        ...
        if (isVeiculo) {
            ...
            detalhes = `${item.marca_modelo || ""} (${item.cor || "cor não informada"})`.trim();
        } else {
...
export async function verAbordagem(id, tipo, appInstance) {
    try {
        ...
        const tabela = tipo === 'veiculo' ? 'abordagens_veiculos' : 'abordagens_pessoas';
